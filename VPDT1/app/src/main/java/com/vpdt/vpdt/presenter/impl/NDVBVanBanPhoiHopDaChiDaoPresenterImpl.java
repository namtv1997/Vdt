...
public class NDVBVanBanPhoiHopDaChiDaoPresenterImpl extends BasePresenterImpl<NDVBVanBanPhoiHopDaChiDaoView> implements NDVBVanBanPhoiHopDaChiDaoPresenter {
    public NDVBVanBanPhoiHopDaChiDaoPresenterImpl(NDVBVanBanPhoiHopDaChiDaoView view) {
        super(view);
    }
    ...
    @Override
    public void duyetVBPhongPhoiHopDaChiDao(int id, int idPhoPhong, String idPhoPhongPhoiHops, String idChuyenVienPhoiHops, String chiDao, String chiDaoChuTri, String hanGiaiQuyet) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().duyetVBPhongPhoiHopDaChiDao(token, id, idPhoPhong, idPhoPhongPhoiHops, idChuyenVienPhoiHops, chiDao, chiDaoChuTri,
    ...
    @Override
    public void tuChoiVBPhoiHopDaChiDao(int idPhoiHop, String noiDungTuChoi) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        NetworkModule.getService().tuChoiVBPhoiHopDaChiDao(token, idPhoiHop, noiDungTuChoi, PrefUtil.getString(getView().gContext(), Key.NAM_LAM_VIEC, ""))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<Response<Boolean>>() {
                    @Override
