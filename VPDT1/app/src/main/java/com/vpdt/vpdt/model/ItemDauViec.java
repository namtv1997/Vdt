...
public class ItemDauViec implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    @SerializedName("moTa")
    ...
    protected ItemDauViec(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<ItemDauViec> CREATOR = new Creator<ItemDauViec>() {
        @Override
        public ItemDauViec createFromParcel(Parcel in) {
            return new ItemDauViec(in);
        }

        @Override
        public ItemDauViec[] newArray(int size) {
            return new ItemDauViec[size];
        }
    };
