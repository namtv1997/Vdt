...
public class NoiDungGiayMoiDaPhanLoaiPresenterImpl extends BasePresenterImpl<NoiDungGiayMoiDaPhanLoaiView> implements NoiDungGiayMoiDaPhanLoaiPresenter {
    public NoiDungGiayMoiDaPhanLoaiPresenterImpl(NoiDungGiayMoiDaPhanLoaiView view) {
        super(view);
    }
    ...
    @Override
    public void duyetGMDaPhanLoai(int id, int idGiamDoc, int idPhoGiamDoc, int idPhongChuTri, String hanGiaiQuyet, String idPhongPhoiHop, String chiDaoGiamDoc, String chiDaoPhoGiamDoc, String chiChuTri, int giamDocDuHop, int PhongDuHop) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().duyetGMDaPhanLoai(token, id, idGiamDoc, idPhoGiamDoc, idPhongChuTri, hanGiaiQuyet, idPhongPhoiHop, chiDaoGiamDoc, chiDaoPhoGiamDoc, chiChuTri, giamDocDuHop, PhongDuHop, PrefUtil.getString(getView().gContext(), Key.NAM_LAM_VIEC, ""))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<Response<Boolean>>() {
                    ...
                    public void onNext(Response<Boolean> booleanResponse) {
                        ...
                        if (booleanResponse.getData()) {
                            getView().duyetGMDaPhanLoaiSuccess();
                        } else {
