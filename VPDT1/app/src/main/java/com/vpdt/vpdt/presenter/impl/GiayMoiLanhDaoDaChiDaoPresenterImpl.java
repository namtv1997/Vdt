...
public class GiayMoiLanhDaoDaChiDaoPresenterImpl extends BasePresenterImpl<GiayMoiLanhDaoDaChiDaoView> implements GiayMoiLanhDaoDaChiDaoPresenter {
    ...
    public GiayMoiLanhDaoDaChiDaoPresenterImpl(GiayMoiLanhDaoDaChiDaoView view) {
        super(view);
    }

    @Override
    public void getallgiaymoilanhdaodaxuly(String nam, boolean isRefresh) {
        if (isRefresh) {
            page = 1;
            isLoadmore = true;
        ...
        NetworkModule.getService().getallgiaymoilanhdaodaxuly(token, nam, uid, page, size)
    ...
    @Override
    public void countgiaymoilanhdaodaxuly(String nam) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().countgiaymoilanhdaodaxuly(token, nam, uid)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
