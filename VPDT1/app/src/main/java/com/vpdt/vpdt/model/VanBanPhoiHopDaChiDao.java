...
public class VanBanPhoiHopDaChiDao implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    protected VanBanPhoiHopDaChiDao(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<VanBanPhoiHopDaChiDao> CREATOR = new Creator<VanBanPhoiHopDaChiDao>() {
        @Override
        public VanBanPhoiHopDaChiDao createFromParcel(Parcel in) {
            return new VanBanPhoiHopDaChiDao(in);
        }

        @Override
        public VanBanPhoiHopDaChiDao[] newArray(int size) {
            return new VanBanPhoiHopDaChiDao[size];
        }
    };
