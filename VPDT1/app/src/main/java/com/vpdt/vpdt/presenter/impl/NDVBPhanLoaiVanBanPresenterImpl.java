...
public class NDVBPhanLoaiVanBanPresenterImpl extends BasePresenterImpl<NDVBPhanLoaiVanBanView> implements NDVBPhanLoaiVanBanPresenter {
    public NDVBPhanLoaiVanBanPresenterImpl(NDVBPhanLoaiVanBanView view) {
        super(view);
    }
    ...
    @Override
    public void themNoidungVBChuaPhanLoai(int id, String hanNoiDung, String noiDung) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().themNoidungVBChuaPhanLoai(token, id, hanNoiDung, noiDung, PrefUtil.getString(getView().gContext(), Key.NAM_LAM_VIEC, ""))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<Response<Boolean>>() {
                    ...
                    public void onNext(Response<Boolean> booleanResponse) {
                        ...
                        if (booleanResponse.getData()) {
                            getView().themNoidungVBChuaPhanLoaiSuccess();
                        } else {
    ...
    @Override
    public void duyetVBChuaPhanLoai(int id, int idGiamDoc, int idPhoGiamDoc, int idPhongChuTri, String hanGiaiQuyet, String idPhongPhoiHop, String chiDaoGiamDoc, String chiDaoPhoGiamDoc, String chiChuTri, int isVBQT) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().duyetVBChuaPhanLoai(token, id, idGiamDoc, idPhoGiamDoc, idPhongChuTri, hanGiaiQuyet, idPhongPhoiHop, chiDaoGiamDoc, chiDaoPhoGiamDoc, chiChuTri, isVBQT, PrefUtil.getString(getView().gContext(), Key.NAM_LAM_VIEC, ""))
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
                .subscribe(new Subscriber<Response<Boolean>>() {
                    @Override
                    ...
                    public void onNext(Response<Boolean> booleanResponse) {
                        Util.getIns().hideLoadding();
                        if (booleanResponse.getData()) {
                            getView().duyetVBChuaPhanLoaiSuccess();
                        } else {
                            Util.showMessenger(booleanResponse.getMessage(), getView().gContext());
                        }
                    }
