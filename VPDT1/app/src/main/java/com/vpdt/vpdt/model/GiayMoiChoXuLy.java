...
public class GiayMoiChoXuLy implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    protected GiayMoiChoXuLy(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<GiayMoiChoXuLy> CREATOR = new Creator<GiayMoiChoXuLy>() {
        @Override
        public GiayMoiChoXuLy createFromParcel(Parcel in) {
            return new GiayMoiChoXuLy(in);
        }

        @Override
        public GiayMoiChoXuLy[] newArray(int size) {
            return new GiayMoiChoXuLy[size];
        }
    };
