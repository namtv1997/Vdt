...
public class VanBanDaPhanLoai implements Parcelable {
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    @SerializedName("isVBQT")
    @Expose
    private Boolean isVBQT;

    protected VanBanDaPhanLoai(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
        ...
        byte tmpIsVBQT = in.readByte();
        isVBQT = tmpIsVBQT == 0 ? null : tmpIsVBQT == 1;
    }

    public static final Creator<VanBanDaPhanLoai> CREATOR = new Creator<VanBanDaPhanLoai>() {
        @Override
        public VanBanDaPhanLoai createFromParcel(Parcel in) {
            return new VanBanDaPhanLoai(in);
        }

        @Override
        public VanBanDaPhanLoai[] newArray(int size) {
            return new VanBanDaPhanLoai[size];
        }
    };
    ...
    public Boolean getIsVBQT() {
        return isVBQT;
    }

    public void setIsVBQT(Boolean isVBQT) {
        this.isVBQT = isVBQT;
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeByte((byte) (isVBQT == null ? 0 : isVBQT ? 1 : 2));
    }
}
