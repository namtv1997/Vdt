...
public class PhongChuTri implements Parcelable{
    @SerializedName("id")
    @Expose
    private Integer id;
    ...
    protected PhongChuTri(Parcel in) {
        if (in.readByte() == 0) {
            id = null;
        } else {
            id = in.readInt();
    ...
    public static final Creator<PhongChuTri> CREATOR = new Creator<PhongChuTri>() {
        @Override
        public PhongChuTri createFromParcel(Parcel in) {
            return new PhongChuTri(in);
        }

        @Override
        public PhongChuTri[] newArray(int size) {
            return new PhongChuTri[size];
        }
    };
