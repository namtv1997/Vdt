...
public class DanhSachTongTheFragment extends BaseFragment<DanhSachCongTacDangPresenter> implements DanhSachCongTacDangView,
    ...
    public int getContentViewId() {
        return R.layout.fragment_danh_sach_tong_the;
    }
    ...
    public void onStart() {
        ...
        try {
            if (TenVB.getText().equals("-- Tất cả --")) {
                getPresenter().getallvbdentongthe(0, 0, NamLamViec, true);
            } else if (TenVB.getText().equals("Văn bản mới nhập")) {
                getPresenter().getallvbdentongthe(0, 1, NamLamViec, true);
            } else if (TenVB.getText().equals("Đã giải quyết")) {
                getPresenter().getallvbdentongthe(0, 2, NamLamViec, true);
            }
            ...
            scrollListener = new EndlessRecyclerViewScrollListener(layoutManager) {
                ...
                public void onLoadMore(int page, int totalItemsCount, RecyclerView view) {
                    if (TenVB.getText().equals("-- Tất cả --")) {
                        getPresenter().getallvbdentongthe(0, 0, NamLamViec, false);
                    } else if (TenVB.getText().equals("Văn bản mới nhập")) {
                        getPresenter().getallvbdentongthe(0, 1, NamLamViec, false);
                    } else if (TenVB.getText().equals("Đã giải quyết")) {
                        getPresenter().getallvbdentongthe(0, 2, NamLamViec, false);
                    }
                }
            };
            ...
            swipe_layout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
                ...
                public void onRefresh() {
                    try {
                        ...
                        Toast.makeText(getActivity(), "Đã có lỗi, thử lại!", Toast.LENGTH_SHORT).show();
    ...
    public void onGetDataSuccess() {
        try {
            ...
            if (TenVB.getText().equals("-- Tất cả --")) {
                getPresenter().countvbden(0, 0, NamLamViec);
            } else if (TenVB.getText().equals("Văn bản mới nhập")) {
                getPresenter().countvbden(0, 1, NamLamViec);
            } else if (TenVB.getText().equals("Đã giải quyết")) {
                getPresenter().countvbden(0, 2, NamLamViec);
            }
        } catch (Exception e) {
            Toast.makeText(getActivity(), "Đã có lỗi, thử lại!", Toast.LENGTH_SHORT).show();
            Objects.requireNonNull(getActivity()).onBackPressed();
    ...
    @OnClick({R.id.TenVB, R.id.btnBack, R.id.vdstt, R.id.lndstt, R.id.rldstt, R.id.btnNamLamViec, R.id.btnHome})
    ...
    public void onItemClickNamLamViec(Integer integer) {
        ...
        if (TenVB.getText().equals("-- Tất cả --")) {
            getPresenter().getallvbdentongthe(0, 0, String.valueOf(integer), true);
        } else if (TenVB.getText().equals("Văn bản mới nhập")) {
            getPresenter().getallvbdentongthe(0, 1, String.valueOf(integer), true);
        } else if (TenVB.getText().equals("Đã giải quyết")) {
            getPresenter().getallvbdentongthe(0, 2, String.valueOf(integer), true);
        }
