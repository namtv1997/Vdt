...
import com.vpdt.vpdt.presenter.NDVBPhanLoaiVanBanPresenter;
import com.vpdt.vpdt.presenter.NDVBPhanLoaiVanBanView;
import com.vpdt.vpdt.presenter.impl.NDVBPhanLoaiVanBanPresenterImpl;
...
public class NDVBPhanLoaiVanBanActivity extends BaseActivity<NDVBPhanLoaiVanBanPresenter> implements NDVBPhanLoaiVanBanView,
    ...
    @BindView(R.id.tvTenPhongGuiLai)
    TextView tvTenPhongGuiLai;
    ...
    @BindView(R.id.cbThongBaoKetLuan)
    CheckBox cbThongBaoKetLuan;
    @BindView(R.id.cbToCongTac)
    CheckBox cbToCongTac;
    @BindView(R.id.cbSTCPhoiHop)
    CheckBox cbSTCPhoiHop;
    ...
    public int getContentViewId() {
        return R.layout.activity_ndvbphan_loai_van_ban;
    }
    ...
    public void initializeComponents() {
        ...
        PhanLoaiVanBan phanLoaiVanBan = getIntent().getParcelableExtra("phanLoaiVanBan");
        if (phanLoaiVanBan != null) {
            if (phanLoaiVanBan.getISTCChuTri() == 1) {
                cbSTCChuTri.setChecked(true);
            }
            if (phanLoaiVanBan.getIVanBanTBKL() == 1) {
                cbThongBaoKetLuan.setChecked(true);
            }
            if (phanLoaiVanBan.getIToCongTac() == 1) {
                cbToCongTac.setChecked(true);
            }
            if (phanLoaiVanBan.getISTCPhoiHop() == 1) {
                cbSTCPhoiHop.setChecked(true);
            }
            if (!phanLoaiVanBan.getNoiDungTuChoi().isEmpty()) {
                tvNoiDungTuChoi.setText("Nội dung từ chối: " + phanLoaiVanBan.getNoiDungTuChoi());
            }
            if (!phanLoaiVanBan.getTenPhongGuiLai().isEmpty()) {
                tvTenPhongGuiLai.setText("Phòng gửi lại: " + phanLoaiVanBan.getTenPhongGuiLai());
            }
            id = phanLoaiVanBan.getId();
            fileDinhKem = phanLoaiVanBan.getUrlFile();
            tvNgay.setText(String.valueOf(phanLoaiVanBan.getNgayNhap()));
            tvSKH.setText(String.valueOf(phanLoaiVanBan.getSoKyHieu()));
            tvSoDen.setText(String.valueOf(phanLoaiVanBan.getSoDen()));
            tvNoigui.setText(String.valueOf(phanLoaiVanBan.getNoiGui()));
            tvNgayThangNam.setText(String.valueOf(phanLoaiVanBan.getHanGiaiQuyet()));
            tvTrichYeu.setText(phanLoaiVanBan.getMoTa() + "." + phanLoaiVanBan.getNoiDung());
            hangiaiquyet = phanLoaiVanBan.getHanGiaiQuyet();
    ...
    @OnClick({R.id.tvChuyenGiamDoc, R.id.tvChuyenPGiamDoc, R.id.tvChonChuTri,
            ...
            R.id.btnBack,
            R.id.btnThemNoiDung, R.id.btnDuyet, R.id.tvFileDinhKem})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tvNgayThangNam:
                ...
                datePickerDialog = new DatePickerDialog(NDVBPhanLoaiVanBanActivity.this,
            ...
            case R.id.btnDuyet:
                ...
                if (cbVbQuanTrong.isChecked()) {
                    ...
                    getPresenter().duyetVBChuaPhanLoai(id, idGiamDoc, idPhoGiamDoc, idPhongChuTri, hangiaiquyet, id_phongphoihop, edtChiDao1.getText().toString(), edtChiDao2.getText().toString(), edtChiDao3.getText().toString(), 1);
                } else {
                    getPresenter().duyetVBChuaPhanLoai(id, idGiamDoc, idPhoGiamDoc, idPhongChuTri, hangiaiquyet, id_phongphoihop, edtChiDao1.getText().toString(), edtChiDao2.getText().toString(), edtChiDao3.getText().toString(), 2);
                }
                break;
            case R.id.btnThemNoiDung:
                ...
                btnNoiDung.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        getPresenter().themNoidungVBChuaPhanLoai(id, tvNgayThangNamDialog.getText().toString(), edtThemYKien.getText().toString());
                    }
                });
    ...
    @Override
    public void themNoidungVBChuaPhanLoaiSuccess() {
        Toast.makeText(this, "Xong", Toast.LENGTH_SHORT).show();
        finish();
    }

    @Override
    public void duyetVBChuaPhanLoaiSuccess() {
        Toast.makeText(this, "Xong", Toast.LENGTH_SHORT).show();
        finish();
    }

    @Override
    public NDVBPhanLoaiVanBanPresenter createPresenter() {
        return new NDVBPhanLoaiVanBanPresenterImpl(this);
    }
