...
public class VanBanGiaoPhongChuChiChoXuLyPresenterImpl extends BasePresenterImpl<VanBanGiaoPhongChuChiChoXuLyView> implements VanBanGiaoPhongChuChiChoXuLyPresenter {
    private int size = 20;
    private int page = 1;
    private boolean isLoadmore = true;
    ArrayList<PhongChuTriChoXL> phongChuTriArrayList = new ArrayList<>();

    public VanBanGiaoPhongChuChiChoXuLyPresenterImpl(VanBanGiaoPhongChuChiChoXuLyView view) {
        super(view);
    }

    @Override
    public void getAllPhongChuTriChoXuLy(String nam, boolean isRefresh) {
        if (isRefresh) {
            page = 1;
            isLoadmore = true;
        ...
        NetworkModule.getService().getAllPhongChuTriChoXuLy(token, nam, page, size)
    ...
    @Override
    public void coutAllPhongChuTriChoXuLy(String nam) {
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getToken(getView().gContext()).getUid();
        NetworkModule.getService().coutAllPhongChuTriChoXuLy(token, nam)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
