...
public class DSVBCuaPhong implements Parcelable {
    @SerializedName("mavb")
    @Expose
    ...
    protected DSVBCuaPhong(Parcel in) {
        if (in.readByte() == 0) {
            mavb = null;
        } else {
            mavb = in.readInt();
    ...
    public static final Creator<DSVBCuaPhong> CREATOR = new Creator<DSVBCuaPhong>() {
        @Override
        public DSVBCuaPhong createFromParcel(Parcel in) {
            return new DSVBCuaPhong(in);
        }

        @Override
        public DSVBCuaPhong[] newArray(int size) {
            return new DSVBCuaPhong[size];
        }
    };
